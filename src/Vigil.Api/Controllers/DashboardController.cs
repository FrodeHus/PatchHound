...
using Vigil.Api.Models.Dashboard;
...
public class DashboardController : ControllerBase
{
    ...
    public DashboardController(VigilDbContext dbContext)
    {
    ...
    public async Task<ActionResult<DashboardSummaryDto>> GetSummary(CancellationToken ct)
    {
        ...
        var bySeverity = await _dbContext.Vulnerabilities
            .AsNoTracking()
        ...
        var vulnsBySeverity = Enum.GetValues<Severity>()
            .ToDictionary(s => s.ToString(), s => bySeverity.FirstOrDefault(x => x.Severity == s)?.Count ?? 0);

        // Vulnerability counts by status
        var byStatus = await _dbContext.Vulnerabilities
            .AsNoTracking()
            .GroupBy(v => v.Status)
        ...
        var vulnsByStatus = Enum.GetValues<VulnerabilityStatus>()
            .ToDictionary(s => s.ToString(), s => byStatus.FirstOrDefault(x => x.Status == s)?.Count ?? 0);

        // SLA compliance
        var tasks = await _dbContext.RemediationTasks
            .AsNoTracking()
            .Select(t => new { t.Status, t.DueDate, t.CreatedAt, t.UpdatedAt })
            .ToListAsync(ct);

        var now = DateTimeOffset.UtcNow;
        var taskTuples = tasks
            .Select(t => (t.Status, t.DueDate))
            .ToList();
        ...
        var vulnerabilityAssetPairs = await _dbContext.VulnerabilityAssets
            .AsNoTracking()
            .Where(va => va.Status != VulnerabilityStatus.Resolved)
            .Join(_dbContext.Vulnerabilities, va => va.VulnerabilityId, v => v.Id, (va, v) => new { v.VendorSeverity, va.AssetId })
            .Join(_dbContext.Assets, x => x.AssetId, a => a.Id, (x, a) => new { x.VendorSeverity, a.Criticality })
            .ToListAsync(ct);
        ...
        var topVulns = await _dbContext.Vulnerabilities
            .AsNoTracking()
            .Where(v => v.Status == VulnerabilityStatus.Open || v.Status == VulnerabilityStatus.InRemediation)
            ...
            .Select(v => new TopVulnerabilityDto(
                ...
                v.PublishedDate.HasValue
                    ? (int)(now - v.PublishedDate.Value).TotalDays
                    : 0))
            .ToListAsync(ct);
    ...
    public async Task<ActionResult<TrendDataDto>> GetTrends(CancellationToken ct)
    {
        ...
        var trends = await _dbContext.Vulnerabilities
            .AsNoTracking()
            .Where(v => v.PublishedDate.HasValue && v.PublishedDate.Value >= twelveMonthsAgo)
            .GroupBy(v => new { v.PublishedDate!.Value.Year, v.PublishedDate!.Value.Month, v.VendorSeverity })
            .Select(g => new
            {
                g.Key.Year,
                g.Key.Month,
        ...
        var items = trends.Select(t => new TrendItem(
