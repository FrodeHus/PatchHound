...
export const Route = createFileRoute('/_authed/workbenches/security-manager/cases/$caseId')({
  loader: ({ params }) => fetchDecisionContext({ data: { caseId: params.caseId } }),
  component: SecurityManagerApprovalCaseRoute,
})

function SecurityManagerApprovalCaseRoute() {
  ...
  const queryKey = ['security-manager-approval-case', selectedTenantId, caseId] as const
  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center py-24 text-muted-foreground">
        Loading security manager workbench...
      </div>
    )
  }
    ...
    <ManagerApprovalCaseWorkbench
      ...
      onResolve={async (action, justification, maintenanceWindowDate) => {
        ...
        try {
          ...
          await queryClient.invalidateQueries({ queryKey: ['security-manager-approval-workbench'] })
        } catch (caught) {
