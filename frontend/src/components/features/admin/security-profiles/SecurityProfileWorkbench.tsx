...
import { ListChecks, ShieldAlert, Sigma } from 'lucide-react'
...
export function SecurityProfileWorkbench({
  ...
  const saveLabel = mode === 'edit' ? 'Apply changes' : 'Create profile'

  return (
    <section className="space-y-5 pb-20">
      <div className="relative overflow-hidden rounded-[1.75rem] border border-white/35 bg-[linear-gradient(125deg,rgba(128,167,255,0.45),rgba(230,188,255,0.5)_50%,rgba(143,246,244,0.35))] px-5 py-5 shadow-[0_24px_80px_rgb(34_24_91_/_0.22)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-foreground/70">
              Define contextual environmental inputs that recalculate CVSS v3.1 for assets assigned to this profile.
            </p>
          </div>
          {overrideCount > 0 ? (
            <Badge className="rounded-full border-orange-400/45 bg-orange-300/15 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-orange-600">
              {overrideCount} override{overrideCount === 1 ? '' : 's'} active
            </Badge>
          ) : null}
        </div>

        <div className="mt-8 grid gap-4 rounded-3xl border border-white/35 bg-white/18 px-5 py-4 backdrop-blur-xl lg:grid-cols-4">
          <ScoreStat label="Base score" value={baseScore.toFixed(1)} badge={cvssSeverity(baseScore)} detail="CVSS v3.1 preview vector" />
          <ScoreStat label="Environmental" value={environmental.score.toFixed(1)} badge={environmental.severity} detail="Recalculated from this profile" />
          <ScoreStat label="Delta from base" value={`${delta >= 0 ? '+' : ''}${delta.toFixed(1)}`} badge={delta > 0 ? 'Raised' : delta < 0 ? 'Lowered' : 'Neutral'} detail={deltaReason(draft)} />
          <ScoreStat label="Tenant scope" value={tenantName} detail={mode === 'edit' ? 'Editing existing profile' : 'New profile'} />
        </div>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <WorkbenchPanel number="1" title="Profile" description="Name and scope. Profiles can be assigned to one or many assets.">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="Applies to">
                <div className="flex h-10 items-center rounded-lg border border-white/35 bg-white/18 px-3 text-sm text-foreground/75">
        ...
        <aside className="space-y-5">
          <Card className="rounded-3xl border-white/35 bg-white/18 shadow-none backdrop-blur-xl">
            <CardContent className="p-5">
              <div className="flex items-center gap-2">
                <span className="flex size-6 items-center justify-center rounded-lg bg-white/45 text-xs">
          ...
          <SummaryPanel title="Breakdown" icon={<ListChecks className="size-3.5" />}>
            <BreakdownRow label="Base" value={baseScore.toFixed(1)} />
            <BreakdownRow label="Environmental" value={environmental.score.toFixed(1)} delta={delta} />
            <BreakdownRow label="Vector" value={environmental.vector.replace('CVSS:3.1/', '')} mono />
          </SummaryPanel>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/30 bg-background/72 px-5 py-3 backdrop-blur-xl">
...
function WorkbenchPanel({
  ...
  return (
    <Card className="rounded-3xl border-white/35 bg-white/20 shadow-none backdrop-blur-xl">
      <CardContent className="space-y-4 p-5">
        <div className="flex items-start gap-3">
          <span className="flex size-6 shrink-0 items-center justify-center rounded-lg bg-white/55 text-xs font-semibold">{number}</span>
          <div>
...
function ScoreStat({ label, value, badge, detail }: { label: string; value: string; badge?: string; detail: string }) {
  return (
    <div className="border-white/25 lg:border-r lg:pr-5 last:border-r-0">
      <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-foreground/50">{label}</p>
      <div className="mt-1 flex min-w-0 items-center gap-2">
        <p className="truncate text-3xl font-semibold tracking-[-0.04em] text-foreground">{value}</p>
        {badge ? <Badge variant="outline" className="rounded-full bg-white/25 px-2 py-0 text-[10px] uppercase">{badge}</Badge> : null}
      </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-3">
        ...
        <p className="text-[11px] text-muted-foreground">{securityProfileRequirementHelp[value]}</p>
      </div>
      <Segmented
        value={value}
...
function Segmented<T extends string>({
  ...
  return (
    <div className="grid rounded-xl border border-white/35 bg-white/18 p-1" style={{ gridTemplateColumns: `repeat(${options.length}, minmax(0, 1fr))` }}>
      {options.map((option) => (
        <button
          key={option}
          type="button"
          ...
          className={[
            'h-9 rounded-lg px-2 text-xs font-medium transition-colors',
            value === option ? 'bg-white/58 text-foreground shadow-sm' : 'text-foreground/65 hover:bg-white/25',
          ].join(' ')}
        >
...
function Field({ label, helper, children }: { label: string; helper?: string; children: ReactNode }) {
  return (
    <label className="grid gap-2">
      ...
      {children}
      {helper ? <span className="text-[11px] leading-4 text-muted-foreground">{helper}</span> : null}
    </label>
  )
}
...
function OptionSelect({
  ...
  return (
    <Select value={value} onValueChange={(next) => next && onValueChange(next)}>
      <SelectTrigger className="h-10 w-full border-white/35 bg-white/25">
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        {options.map((option) => (
    ...
    <Card className="rounded-3xl border-white/35 bg-white/18 shadow-none backdrop-blur-xl">
      <CardContent className="p-5">
        <div className="mb-4 flex items-center gap-2">
          <span className="flex size-6 items-center justify-center rounded-lg bg-white/45 text-xs">{icon}</span>
          <h2 className="text-sm font-semibold">{title}</h2>
        </div>
...
function BreakdownRow({ label, value, delta, mono = false }: { label: string; value: string; delta?: number; mono?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-foreground/10 pb-2 last:border-0 last:pb-0">
      <span className="text-xs text-muted-foreground">{label}</span>
      <span className={['max-w-[13rem] text-right text-xs font-medium text-foreground', mono ? 'font-mono leading-5' : ''].join(' ')}>
        {value}
        {typeof delta === 'number' ? (
          <span className={delta > 0 ? 'ml-2 text-red-500' : delta < 0 ? 'ml-2 text-emerald-500' : 'ml-2 text-muted-foreground'}>
...
function ScoreGauge({ score }: { score: number }) {
  ...
  return (
    <div className="mx-auto mt-4 grid aspect-[2/1] w-full max-w-[260px] place-items-end overflow-hidden">
      ...
      >
        <div className="absolute inset-x-[11%] bottom-0 aspect-[2/1] rounded-t-full bg-background/70" />
        <div
        ...
        <div className="absolute inset-x-0 bottom-2 text-center">
          <p className="text-5xl font-semibold tracking-[-0.08em]">{score.toFixed(1)}</p>
        </div>
      </div>
    </div>
  )
